...
// ============================================
const EXTRA_TIME_THRESHOLD = 1;        // Déclencher uniquement à 1 seconde ou moins
const EXTRA_TIME_DURATION = 30;        // Ajouter 30 secondes (pas 59)
...
function shouldTriggerExtraTime(room, lotNum, currentTime) {
  // Ne déclencher que si temps restant <= 1 seconde
  ...
  log(`[EXTRA TIME] Condition remplie pour lot ${lotNum} - Temps: ${currentTime}s <= 1s`);

  return true;
}

function calculateExtraTime(currentTime, extraTimeCount) {
  // Extra time de 30 secondes (pas 59)
...
function registerRoomHandler(io, socket) {
  ...
  socket.on('getMsgRoom', (data) => {
    ...
    // UNIQUEMENT SI triggerExtraTime = true ET temps <= 1s
    if (data.type === 'doEncheres' && !isAdmin && data.msg) {
      ...
      // Vérifier si on doit déclencher extra time (seulement à 1s ou moins)
      const shouldTrigger = clientRequestExtraTime && shouldTriggerExtraTime(data.room, lotNum, currentTime);

      if (shouldTrigger) {
        const lotState = getLotState(data.room, lotNum);
        const newTime = calculateExtraTime(currentTime, lotState.extraTimeCount);

        // Mettre à jour le timer (extra time)
        ...
        // Enchère normale : NE PAS modifier le timer
        log(`[ENCHERE] Lot ${lotNum} - Enchere normale, timer inchangé (${currentTime}s)`);
    ...
    if (data.type === 'doEncheres' && data.msg && !isAdmin) {
      ...
      if (room) {
        ...
        if (shouldTrigger) {
          ...
          log(`[ENCHERE PRIVEE] Lot ${lotNum} - Enchere normale, timer inchangé`);
          finalMsg.timerUnchanged = true;
        }
      }
